...
class WorkflowService:
    ...
    def __init__(self, workflows_dir: Path, workflow_engine: WorkflowEngine):
        ...
        self.templates_dir = workflows_dir / "templates"
    ...
    async def list_templates(self) -> List[Dict[str, Any]]:
        ...
        Example:
            >>> service = WorkflowService(Path("workflows/v2"), engine)
        ...
        for file in self.templates_dir.glob("*.json"):
            try:
                ...
                templates.append({
                    "template_id": template_data.get("template_id", file.stem),
                    "name": template_data.get("name", file.stem),
                    "description": template_data.get("description", ""),
                    "category": template_data.get("category", "general"),
                    "tags": template_data.get("tags", []),
                    "version": template_data.get("version", "1.0.0"),
                    "author": template_data.get("author", "unknown"),
                    "nodeCount": len(template_data.get("workflow", {}).get("nodes", [])),
                    "edgeCount": len(template_data.get("workflow", {}).get("edges", []))
                })
            except (json.JSONDecodeError, IOError, OSError) as e:
                logger.warning(f"Skipping invalid template file {file.name}: {e}")
                continue

        logger.info(f"Listed {len(templates)} workflow templates")
        return templates
    ...
    async def get_template(self, template_id: str) -> Dict[str, Any]:
        ...
        Args:
            template_id: Template ID (e.g., "basic-recon")

        Returns:
            Full template definition including workflow

        Raises:
            NotFoundError: If template not found
        """
        # SECURITY: Validate template_id BEFORE any string manipulation
        # to prevent path traversal (e.g., "../../../etc/passwd")
        self._validate_filename(template_id)

        # Template filename is template_id.json
        ...
        template_data = json.loads(file_path.read_text())
